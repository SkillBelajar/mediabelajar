...
/**
 * Table class for live
 */
class Live extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // id_live
        $this->id_live = new DbField('live', 'live', 'x_id_live', 'id_live', '`id_live`', '`id_live`', 3, 100, -1, false, '`id_live`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->id_live->IsAutoIncrement = true; // Autoincrement field
        $this->id_live->IsPrimaryKey = true; // Primary key field
        $this->id_live->Sortable = false; // Allow sort
        $this->id_live->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['id_live'] = &$this->id_live;

        // aksi
        $this->aksi = new DbField('live', 'live', 'x_aksi', 'aksi', '`aksi`', '`aksi`', 200, 100, -1, false, '`aksi`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->aksi->Nullable = false; // NOT NULL field
        $this->aksi->Required = true; // Required field
        $this->aksi->Sortable = true; // Allow sort
        ...
        // live_catatan
        $this->live_catatan = new DbField('live', 'live', 'x_live_catatan', 'live_catatan', '`live_catatan`', '`live_catatan`', 201, 65535, -1, false, '`live_catatan`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        $this->live_catatan->Nullable = false; // NOT NULL field
        $this->live_catatan->Required = true; // Required field
        $this->live_catatan->Sortable = true; // Allow sort
        $this->Fields['live_catatan'] = &$this->live_catatan;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`live`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_live->setDbValue($conn->lastInsertId());
            $rs['id_live'] = $this->id_live->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_live', $rs)) {
                AddFilter($where, QuotedName('id_live', $this->Dbid) . '=' . QuotedValue($rs['id_live'], $this->id_live->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_live', $row) ? $row['id_live'] : null;
        } else {
            $val = $this->id_live->OldValue !== null ? $this->id_live->OldValue : $this->id_live->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_live@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "LiveEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "LiveAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("LiveView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("LiveView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "LiveAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "LiveAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("LiveEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("LiveAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("LiveDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_live:" . JsonEncode($this->id_live->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_live->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_live->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_live") ?? Route("id_live")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_live->CurrentValue = $key;
            } else {
                $this->id_live->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->live_catatan->EditValue = $this->live_catatan->CurrentValue;
