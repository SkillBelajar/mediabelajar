...
class Media extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // nama_media
        $this->nama_media = new DbField('media', 'media', 'x_nama_media', 'nama_media', '`nama_media`', '`nama_media`', 200, 255, -1, false, '`nama_media`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->nama_media->Nullable = false; // NOT NULL field
        $this->nama_media->Required = true; // Required field
        $this->nama_media->Sortable = true; // Allow sort
        $this->Fields['nama_media'] = &$this->nama_media;

        // aktif
        $this->aktif = new DbField('media', 'media', 'x_aktif', 'aktif', '`aktif`', '`aktif`', 200, 100, -1, false, '`aktif`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->aktif->Lookup = new Lookup('aktif', 'media', false, '', ["","","",""], [], [], [], [], [], [], '', '');
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_media->setDbValue($conn->lastInsertId());
            $rs['id_media'] = $this->id_media->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_media', $rs)) {
                AddFilter($where, QuotedName('id_media', $this->Dbid) . '=' . QuotedValue($rs['id_media'], $this->id_media->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->nama_media->DbValue = $row['nama_media'];
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_media', $row) ? $row['id_media'] : null;
        } else {
            $val = $this->id_media->OldValue !== null ? $this->id_media->OldValue : $this->id_media->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_media@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "MediaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "MediaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("MediaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("MediaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "MediaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "MediaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        return $this->keyUrl("MediaDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_media:" . JsonEncode($this->id_media->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_media->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_media->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_media") ?? Route("id_media")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_media->CurrentValue = $key;
            } else {
                $this->id_media->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->nama_media->EditValue = $this->nama_media->CurrentValue;
