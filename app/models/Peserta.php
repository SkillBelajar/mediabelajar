...
class Peserta extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // tanggal_jam
        $this->tanggal_jam = new DbField('peserta', 'peserta', 'x_tanggal_jam', 'tanggal_jam', '`tanggal_jam`', '`tanggal_jam`', 200, 100, -1, false, '`tanggal_jam`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // nama_peserta
        $this->nama_peserta = new DbField('peserta', 'peserta', 'x_nama_peserta', 'nama_peserta', '`nama_peserta`', '`nama_peserta`', 200, 100, -1, false, '`nama_peserta`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // benar
        $this->benar = new DbField('peserta', 'peserta', 'x_benar', 'benar', '`benar`', '`benar`', 200, 100, -1, false, '`benar`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->benar->Nullable = false; // NOT NULL field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`peserta`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_peserta->setDbValue($conn->lastInsertId());
            $rs['id_peserta'] = $this->id_peserta->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_peserta', $rs)) {
                AddFilter($where, QuotedName('id_peserta', $this->Dbid) . '=' . QuotedValue($rs['id_peserta'], $this->id_peserta->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->id_peserta->DbValue = $row['id_peserta'];
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_peserta', $row) ? $row['id_peserta'] : null;
        } else {
            $val = $this->id_peserta->OldValue !== null ? $this->id_peserta->OldValue : $this->id_peserta->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_peserta@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "id_peserta:" . JsonEncode($this->id_peserta->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_peserta->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_peserta->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_peserta") ?? Route("id_peserta")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_peserta->CurrentValue = $key;
            } else {
                $this->id_peserta->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->tanggal_jam->EditValue = $this->tanggal_jam->CurrentValue;
