...
class Siswa extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // nama
        $this->nama = new DbField('siswa', 'siswa', 'x_nama', 'nama', '`nama`', '`nama`', 201, 65535, -1, false, '`nama`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        ...
        // kelas
        $this->kelas = new DbField('siswa', 'siswa', 'x_kelas', 'kelas', '`kelas`', '`kelas`', 200, 100, -1, false, '`kelas`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`siswa`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_siswa->setDbValue($conn->lastInsertId());
            $rs['id_siswa'] = $this->id_siswa->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_siswa', $rs)) {
                AddFilter($where, QuotedName('id_siswa', $this->Dbid) . '=' . QuotedValue($rs['id_siswa'], $this->id_siswa->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_siswa', $row) ? $row['id_siswa'] : null;
        } else {
            $val = $this->id_siswa->OldValue !== null ? $this->id_siswa->OldValue : $this->id_siswa->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_siswa@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        global $Language;
        if ($pageName == "SiswaView") {
            return $Language->phrase("View");
        } elseif ($pageName == "SiswaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SiswaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SiswaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SiswaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SiswaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "SiswaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("SiswaEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SiswaAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SiswaDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_siswa:" . JsonEncode($this->id_siswa->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_siswa->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_siswa->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_siswa") ?? Route("id_siswa")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function renderEditRow()
    {
        ...
        // id_siswa
        $this->id_siswa->EditAttrs["class"] = "form-control";
