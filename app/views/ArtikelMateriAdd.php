...
// Page object
$ArtikelMateriAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fartikel_materiadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fartikel_materiadd = currentForm = new ew.Form("fartikel_materiadd", "add");

    // Add fields
    var fields = ew.vars.tables.artikel_materi.fields;
    fartikel_materiadd.addFields([
    ...
    // Validate form
    fartikel_materiadd.validate = function () {
    ...
    // Form_CustomValidate
    fartikel_materiadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fartikel_materiadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fartikel_materiadd.lists.id_materi = <?= $Page->id_materi->toClientList($Page) ?>;
    loadjs.done("fartikel_materiadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fartikel_materiadd" id="fartikel_materiadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fartikel_materiadd", "editor"], function() {
	ew.createEditor("fartikel_materiadd", "x_isi", 35, 4, <?= $Page->isi->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
