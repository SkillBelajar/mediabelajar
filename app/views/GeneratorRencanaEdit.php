...
// Page object
$GeneratorRencanaEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fgenerator_rencanaedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fgenerator_rencanaedit = currentForm = new ew.Form("fgenerator_rencanaedit", "edit");

    // Add fields
    var fields = ew.vars.tables.generator_rencana.fields;
    fgenerator_rencanaedit.addFields([
    ...
    // Validate form
    fgenerator_rencanaedit.validate = function () {
    ...
    // Form_CustomValidate
    fgenerator_rencanaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgenerator_rencanaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fgenerator_rencanaedit.lists.id_indikator_rencana = <?= $Page->id_indikator_rencana->toClientList($Page) ?>;
    loadjs.done("fgenerator_rencanaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fgenerator_rencanaedit" id="fgenerator_rencanaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_generator_rencana->Visible) { // id_generator_rencana ?>
    <div id="r_id_generator_rencana" class="form-group row">
        <label id="elh_generator_rencana_id_generator_rencana" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_generator_rencana->caption() ?><?= $Page->id_generator_rencana->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_generator_rencana->cellAttributes() ?>>
<span id="el_generator_rencana_id_generator_rencana">
<span<?= $Page->id_generator_rencana->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_generator_rencana->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="generator_rencana" data-field="x_id_generator_rencana" name="x_id_generator_rencana" id="x_id_generator_rencana" value="<?= HtmlEncode($Page->id_generator_rencana->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fgenerator_rencanaedit", "editor"], function() {
	ew.createEditor("fgenerator_rencanaedit", "x_isi", 35, 4, <?= $Page->isi->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
