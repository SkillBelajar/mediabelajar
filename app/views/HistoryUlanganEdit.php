...
// Page object
$HistoryUlanganEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fhistory_ulanganedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fhistory_ulanganedit = currentForm = new ew.Form("fhistory_ulanganedit", "edit");

    // Add fields
    var fields = ew.vars.tables.history_ulangan.fields;
    fhistory_ulanganedit.addFields([
        ["id_history_ulangan", [fields.id_history_ulangan.required ? ew.Validators.required(fields.id_history_ulangan.caption) : null], fields.id_history_ulangan.isInvalid],
    ...
    // Validate form
    fhistory_ulanganedit.validate = function () {
    ...
    // Form_CustomValidate
    fhistory_ulanganedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fhistory_ulanganedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fhistory_ulanganedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fhistory_ulanganedit" id="fhistory_ulanganedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_history_ulangan->Visible) { // id_history_ulangan ?>
    <div id="r_id_history_ulangan" class="form-group row">
        <label id="elh_history_ulangan_id_history_ulangan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_history_ulangan->caption() ?><?= $Page->id_history_ulangan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_history_ulangan->cellAttributes() ?>>
<span id="el_history_ulangan_id_history_ulangan">
<span<?= $Page->id_history_ulangan->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_history_ulangan->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="history_ulangan" data-field="x_id_history_ulangan" name="x_id_history_ulangan" id="x_id_history_ulangan" value="<?= HtmlEncode($Page->id_history_ulangan->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
