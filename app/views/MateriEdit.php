...
loadjs.ready("head", function () {
    ...
    fmateriedit = currentForm = new ew.Form("fmateriedit", "edit");
    ...
    // Form_CustomValidate
    fmateriedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmateriedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmateriedit.lists.id_media = <?= $Page->id_media->toClientList($Page) ?>;
    loadjs.done("fmateriedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmateriedit" id="fmateriedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_materi->Visible) { // id_materi ?>
    <div id="r_id_materi" class="form-group row">
        <label id="elh_materi_id_materi" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_materi->caption() ?><?= $Page->id_materi->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_materi->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="materi" data-field="x_id_materi" name="x_id_materi" id="x_id_materi" value="<?= HtmlEncode($Page->id_materi->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
