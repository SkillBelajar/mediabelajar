...
loadjs.ready("head", function () {
    ...
    fmediaedit = currentForm = new ew.Form("fmediaedit", "edit");

    // Add fields
    var fields = ew.vars.tables.media.fields;
    fmediaedit.addFields([
    ...
    // Form_CustomValidate
    fmediaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmediaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmediaedit.lists.aktif = <?= $Page->aktif->toClientList($Page) ?>;
    loadjs.done("fmediaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmediaedit" id="fmediaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_media->Visible) { // id_media ?>
    <div id="r_id_media" class="form-group row">
        <label id="elh_media_id_media" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_media->caption() ?><?= $Page->id_media->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_media->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="media" data-field="x_id_media" name="x_id_media" id="x_id_media" value="<?= HtmlEncode($Page->id_media->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php
    if (in_array("materi", explode(",", $Page->getCurrentDetailTable())) && $materi->DetailEdit) {
?>
