...
loadjs.ready("head", function () {
    ...
    fopen_slideedit = currentForm = new ew.Form("fopen_slideedit", "edit");

    // Add fields
    var fields = ew.vars.tables.open_slide.fields;
    fopen_slideedit.addFields([
        ["id_open_slide", [fields.id_open_slide.required ? ew.Validators.required(fields.id_open_slide.caption) : null], fields.id_open_slide.isInvalid],
    ...
    // Validate form
    fopen_slideedit.validate = function () {
    ...
    // Form_CustomValidate
    fopen_slideedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fopen_slideedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fopen_slideedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fopen_slideedit" id="fopen_slideedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_open_slide->Visible) { // id_open_slide ?>
    <div id="r_id_open_slide" class="form-group row">
        <label id="elh_open_slide_id_open_slide" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_open_slide->caption() ?><?= $Page->id_open_slide->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_open_slide->cellAttributes() ?>>
<span id="el_open_slide_id_open_slide">
<span<?= $Page->id_open_slide->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_open_slide->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="open_slide" data-field="x_id_open_slide" name="x_id_open_slide" id="x_id_open_slide" value="<?= HtmlEncode($Page->id_open_slide->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
