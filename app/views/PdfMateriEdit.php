...
loadjs.ready("head", function () {
    ...
    fpdf_materiedit = currentForm = new ew.Form("fpdf_materiedit", "edit");

    // Add fields
    var fields = ew.vars.tables.pdf_materi.fields;
    fpdf_materiedit.addFields([
    ...
    // Validate form
    fpdf_materiedit.validate = function () {
    ...
    // Form_CustomValidate
    fpdf_materiedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpdf_materiedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpdf_materiedit.lists.id_materi = <?= $Page->id_materi->toClientList($Page) ?>;
    loadjs.done("fpdf_materiedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpdf_materiedit" id="fpdf_materiedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_pdf_materi->Visible) { // id_pdf_materi ?>
    <div id="r_id_pdf_materi" class="form-group row">
        <label id="elh_pdf_materi_id_pdf_materi" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_pdf_materi->caption() ?><?= $Page->id_pdf_materi->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_pdf_materi->cellAttributes() ?>>
<span id="el_pdf_materi_id_pdf_materi">
<span<?= $Page->id_pdf_materi->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_pdf_materi->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="pdf_materi" data-field="x_id_pdf_materi" name="x_id_pdf_materi" id="x_id_pdf_materi" value="<?= HtmlEncode($Page->id_pdf_materi->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_file_pdf" id= "fa_x_file_pdf" value="<?= (Post("fa_x_file_pdf") == "0") ? "0" : "1" ?>">
