...
// Page object
$RencanaPembelajaranEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var frencana_pembelajaranedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    frencana_pembelajaranedit = currentForm = new ew.Form("frencana_pembelajaranedit", "edit");

    // Add fields
    var fields = ew.vars.tables.rencana_pembelajaran.fields;
    frencana_pembelajaranedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = frencana_pembelajaranedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    frencana_pembelajaranedit.validate = function () {
    ...
    // Form_CustomValidate
    frencana_pembelajaranedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frencana_pembelajaranedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frencana_pembelajaranedit.lists.id_indikator = <?= $Page->id_indikator->toClientList($Page) ?>;
    frencana_pembelajaranedit.lists.id_materi = <?= $Page->id_materi->toClientList($Page) ?>;
    frencana_pembelajaranedit.lists.tampilkan = <?= $Page->tampilkan->toClientList($Page) ?>;
    loadjs.done("frencana_pembelajaranedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="frencana_pembelajaranedit" id="frencana_pembelajaranedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_rencana_pembelajaran->Visible) { // id_rencana_pembelajaran ?>
    <div id="r_id_rencana_pembelajaran" class="form-group row">
        <label id="elh_rencana_pembelajaran_id_rencana_pembelajaran" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_rencana_pembelajaran->caption() ?><?= $Page->id_rencana_pembelajaran->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_rencana_pembelajaran->cellAttributes() ?>>
<span id="el_rencana_pembelajaran_id_rencana_pembelajaran">
<span<?= $Page->id_rencana_pembelajaran->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_rencana_pembelajaran->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="rencana_pembelajaran" data-field="x_id_rencana_pembelajaran" name="x_id_rencana_pembelajaran" id="x_id_rencana_pembelajaran" value="<?= HtmlEncode($Page->id_rencana_pembelajaran->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["frencana_pembelajaranedit", "editor"], function() {
	ew.createEditor("frencana_pembelajaranedit", "x_kegiatan", 35, 4, <?= $Page->kegiatan->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
