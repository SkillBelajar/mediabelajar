...
loadjs.ready("head", function () {
    ...
    fvideoedit = currentForm = new ew.Form("fvideoedit", "edit");

    // Add fields
    var fields = ew.vars.tables.video.fields;
    fvideoedit.addFields([
        ["id_video", [fields.id_video.required ? ew.Validators.required(fields.id_video.caption) : null], fields.id_video.isInvalid],
    ...
    // Form_CustomValidate
    fvideoedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fvideoedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fvideoedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fvideoedit" id="fvideoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_video->Visible) { // id_video ?>
    <div id="r_id_video" class="form-group row">
        <label id="elh_video_id_video" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_video->caption() ?><?= $Page->id_video->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_video->cellAttributes() ?>>
<span id="el_video_id_video">
<span<?= $Page->id_video->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_video->EditValue)) ?>"></span>
</span>
<input type="hidden" data-table="video" data-field="x_id_video" name="x_id_video" id="x_id_video" value="<?= HtmlEncode($Page->id_video->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
